...
综上，本文的贡献在于把指数增强的核心组合优化算法做 ``深''：在数值稳健的约束优化基础上，进一步将具有理论保证的预测不确定性纳入优化，并以严谨的实证刻画其在 A 股低信噪比环境下的有效性边界与改进方向。图~\ref{fig:framework} 以三层研究框架的形式直观呈现上述工作的问题、内容与目标。

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
  hblue/.style ={draw=cblue1!55, fill=lblue1,   rounded corners=2pt,
                 align=center, font=\small\bfseries,
                 minimum width=2.8cm, minimum height=0.56cm},
  hgreen/.style={draw=cgreen!80, fill=cgreen!35, rounded corners=2pt,
                 align=center, font=\small\bfseries,
                 minimum width=2.8cm, minimum height=0.56cm},
  hgray/.style ={draw=black!35, fill=black!8,   rounded corners=2pt,
                 align=center, font=\small\bfseries,
                 minimum width=2.8cm, minimum height=0.56cm},
  %% 问题框：固定高度 1.6cm，宽 3.0cm
  pbox/.style={draw=black!35, rounded corners=3pt, align=center, fill=white,
               text width=2.8cm, minimum height=1.6cm, font=\small},
  %% 研究内容框：宽度加大至 4.4cm，减少折行
  cbox/.style={draw=black!35, rounded corners=3pt, align=center, fill=white,
               text width=4.2cm, minimum height=0.9cm, font=\small},
  %% 目标框：大圆角矩形
  tbox/.style={draw=black!35, rounded corners=14pt, align=center, fill=white,
               text width=2.2cm, minimum height=2.0cm, font=\small},
  arr/.style={-{Latex[length=2mm]}, thick, color=cblue1!65},
]

%% ── 列坐标：x = 2.0 / 7.2 / 12.2 ─────────────────────────
%% ── 行坐标（中心）：y = -0.7 / -2.5 / -4.3 / -6.0 ─────────
%%    构建层中心 -0.7；决策层两框分别 -2.5 / -4.3；执行层 -6.0

%% ── 列标题 ─────────────────────────────────────────────────
\node[hblue]  at (2.0,  0.5) {问题与挑战};
\node[hgreen] at (7.2,  0.5) {研究内容};
\node[hgray]  at (12.2, 0.5) {实现目标};

%% ── 行标签 ─────────────────────────────────────────────────
\node[font=\small, anchor=east] at (-0.1, -0.7) {构建层};
\node[font=\small, anchor=east] at (-0.1, -3.4) {决策层};
\node[font=\small, anchor=east] at (-0.1, -6.0) {执行层};

%% ── 问题框 ─────────────────────────────────────────────────
\node[pbox] (p1) at (2.0, -0.7) {因子来源单一，\\语义信息不足};
\node[pbox] (p2) at (2.0, -3.4) {单模型鲁棒性有限，\\不确定性难进入\\组合优化};
\node[pbox] (p3) at (2.0, -6.0) {回测计算耗时长，\\策略迭代效率受限};

%% ── 研究内容框（四框，y 间距 1.7cm 保证不重叠）──────────────
\node[cbox] (c1)  at (7.2, -0.7) {设计 LLM 舆情情感因子管线};
\node[cbox] (c2a) at (7.2, -2.5) {构建异构 Stacking 集成\\LGB/XGB/GRU/Ridge $+$ MLP};
\node[cbox] (c2b) at (7.2, -4.3) {设计 Conformal 置信加权组合优化\\（Split/Mondrian $+$ QP）};
\node[cbox] (c3)  at (7.2, -6.0) {实现 Numba JIT $+$ joblib 并行回测引擎};

%% ── 目标框 ─────────────────────────────────────────────────
\node[tbox] (t1) at (12.2, -2.8) {稳健超额\\IR 提升、\\回撤收敛};
\node[tbox] (t2) at (12.2, -5.5) {高效可复现\\CPU 友好、\\迭代加速};

%% ── 箭头：问题 → 研究内容 ──────────────────────────────────
\draw[arr] (p1.east) -- (c1.west);
\draw[arr] (p2.east) -- (c2a.west);
\draw[arr] (p2.east) -- (c2b.west);
\draw[arr] (p3.east) -- (c3.west);

%% ── 箭头：研究内容 → 目标 ──────────────────────────────────
\draw[arr] (c1.east)  -- (t1.west);
\draw[arr] (c2a.east) -- (t1.west);
\draw[arr] (c2b.east) -- (t1.west);
\draw[arr] (c3.east)  -- (t2.west);

\end{tikzpicture}
\caption{本文三层研究框架：问题与挑战、研究内容与实现目标}
\label{fig:framework}
\end{figure}
...
\end{tikzpicture}
\caption{指数增强系统整体架构（虚线框为本文核心算法，灰色虚线为闭环反馈路径）}
\label{fig:arch}
\end{figure}

在该框架中，本文的核心算法位于图~\ref{fig:arch} 的虚线框内，即不确定性量化层与组合优化层。alpha 预测层提供输入信号，回测评估层提供输出度量，二者为核心算法的设计与评估提供了完整的上下文。值得强调的是，该系统并非单向流水线，而是一个随市场状态持续自适应的\emph{闭环}：回测评估层产出的已实现收益并非流程终点，在月度滚动机制下，每个调仓周期新实现的超额收益会作为新标签反馈到 alpha 预测层与不确定性量化层（对应图~\ref{fig:arch} 中的虚线反馈箭头），驱动 LightGBM 在最新窗口上重训、Conformal 在最新校准残差上重新估计置信度，从而使预测与不确定性度量始终贴合近期的市场风格；这一反馈迭代机制也与第四章严格样本外实验揭示的 A 股强非平稳性相呼应。在此闭环之内，alpha 预测层与组合优化层又保持\emph{解耦}：优化算法只接收标准化的 alpha 分数与（可选的）置信度，而不依赖具体的预测模型，这使得算法可独立替换上游模型、便于消融与复用。
