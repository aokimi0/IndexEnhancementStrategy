...
\draw[arr] (cp)--(opt);
\draw[arr] (opt)--(bt);
% 月度滚动反馈闭环：回测评估层 -> alpha 预测层
\draw[arr, dashed] (bt.east) -- ++(0.6,0) |- (alpha.east);
\node[anchor=west, font=\scriptsize, align=left] at ($(alpha.east)!0.5!(bt.east)+(0.72,0)$)
  {月度滚动反馈：\\实现收益作为新标签\\重训 LightGBM、再校准 Conformal};
\begin{scope}[on background layer]
\node[draw, dashed, rounded corners, fit=(cp)(opt), inner sep=0.22cm] (corebox) {};
\end{scope}
\end{tikzpicture}%
}
\caption{指数增强系统整体架构（虚线框为本文核心算法；虚线箭头为月度滚动重训与再校准的反馈闭环）}
\label{fig:arch}
\end{figure}
